...
*** Actualizar versión (Mediante PPA)
Si la distribución no permite actualizar a la última versión de Emacs, prefieres usar apt y quieres que Emacs se sienta como una aplicación nativa del sistema, lo mejor es usar un PPA de confianza. El de Kevin Kelley es un clásico, aunque últimamente el de Ubuntu Handbook suele estar más al día con las versiones estables.
