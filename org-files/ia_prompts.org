...
Para realizar modificaciones en la app primero hay que pasar el contexto, 
#+begin_src
Soy el desarrollador de Sherlocaster, una aplicación dockerizada en Python que automatiza la creación de podcasts. La app descarga audio de YouTube, Twitch y Kick, lo procesa a mono (64kbps), genera un feed RSS compatible y lo sube a un almacenamiento remoto mediante rclone.
...
Ejecutamos este comando desde Linux Mint acceciendo en remoto al server conde corre docker y copiando la salida directamente al portapapeles.

#+begin_quote
Imprecindible tener instalado Xclip ~~sudo apt intall xclip~~
#+end_quote
